...
def read_hardware_version(esp,efuses, args):

    print("Hardware Version: {}".format(efuses.read_efuse(30)))

def burn_hardware_version(esp, efuses, args):
    num_bytes = efuses.get_block_len()
    offset = 0
    data = args.data
    ...
    if not args.force_write_always and \
       efuse.get_raw() != b'\x00' * num_bytes:
        print("Hardware version already has values written: {}" .format(efuses.read_efuse(30)))
        return 
    ...
    print("Burn efuse for hardware version:{} PASS!" .format(version.hex()))
...
def main():
    ...
    # 写硬件版本号
    p = subparsers.add_parser('burn_hardware_version',
                              help="burn_hardware_version")                     
    # p.add_argument('--offset', '-o', help='Byte offset in the efuse block', type=int, default=0)
    # p.add_argument('block', help='Efuse block to burn.', choices=["BLK1","BLK2","BLK3"])
    p.add_argument('data', action="store",type =int)
    add_force_write_always(p)
    #  

    # 读硬件版本号
    p = subparsers.add_parser('read_hardware_version',
                              help="read_hardware_version")                     
    # p.add_argument('--offset', '-o', help='Byte offset in the efuse block', type=int, default=0)
    # p.add_argument('block', help='Efuse block to burn.', choices=["BLK1","BLK2","BLK3"])
    # p.add_argument('data', action="store",type =int)
    add_force_write_always(p)
