...
def record():
    print("record_1")
    audio.record('1.wav', 2, 8, 2, 16000)
    print("record_2")
    time.sleep(2)
    print("record_3")
    audio.play('1.wav')
...
P.set_event_cb(on_touchpad_p_pressed)
Y.set_event_cb(on_touchpad_y_pressed)
T.set_event_cb(on_touchpad_t_pressed)
H.set_event_cb(on_touchpad_h_pressed)
O.set_event_cb(on_touchpad_o_pressed)
N.set_event_cb(on_touchpad_n_pressed)
...
def ShowSensor():
    while True:
        if g_isOpenCamera:
            ...
            time.sleep(2)
            lcd.draw_color(lcd.WHITE)  # 白
            time.sleep(2)
            lcd.draw_color(lcd.BLACK)  # 黑
